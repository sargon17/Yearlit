...
public struct CustomCalendar: Codable, Identifiable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    reminderTimeZone =
      try container.decodeIfPresent(String.self, forKey: .reminderTimeZone) ?? TimeZone.current.identifier
    notificationPrivacyMode =
      try container.decodeIfPresent(NotificationPrivacyMode.self, forKey: .notificationPrivacyMode) ?? .full
...
public final class CustomCalendarStore: ObservableObject {
  ...
  public func addEntry(calendarId: UUID, entry: CalendarEntry) {
    do {
      ...
      try finishEntryMutationPublishingSnapshot(
        calendarId: calendarId,
        dayKey: dayKey,
        updatedEntry: CalendarEntry(
          date: canonicalDate,
          count: entry.count,
          completed: entry.completed
        ),
    ...
    do {
      ...
      case .binary:
        if let existingEntry {
          ...
          let normalizedEntry = CalendarEntry(date: canonicalDate, count: 1, completed: true)
          let entryEntity = CalendarEntryEntity(
            compositeKey: compositeKey,
            calendarId: calendarId,
            dayKey: dayKey,
  ...
  public func clearEntries(calendarId: UUID) {
    do {
      ...
      try finishCalendarMutationPublishingSnapshot(calendarId: calendarId, in: context) { calendar in
        calendar.entries = [:]
      }
    } catch {
  ...
  public func replaceEntries(calendarId: UUID, entries: [String: CalendarEntry]) {
    do {
      let context = makeContext()
      guard let calendarEntity = fetchCalendarEntity(id: calendarId, in: context) else { return }
      let cadence = CalendarCadence(rawValue: calendarEntity.cadenceRawValue) ?? .daily
      ...
      var normalizedEntries: [String: CalendarEntry] = [:]
      for entry in entries.values {
        let canonicalDate = canonicalEntryDate(for: entry.date, cadence: cadence)
        let dayKey = formatDate(date: canonicalDate, cadence: cadence)
        normalizedEntries[dayKey] = CalendarEntry(
          date: canonicalDate,
          count: entry.count,
          completed: entry.completed
        )
      ...
      for (dayKey, entry) in normalizedEntries {
        context.insert(
          CalendarEntryEntity(
      ...
      try finishCalendarMutationPublishingSnapshot(calendarId: calendarId, in: context) { calendar in
        calendar.entries = normalizedEntries
      }
    } catch {
      NSLog("Failed to replace entries: \(error)")
    }
  }
  ...
  private func finishEntryMutationPublishingSnapshot(
    ...
    try finishCalendarMutationPublishingSnapshot(calendarId: calendarId, in: context) { calendar in
      if let updatedEntry {
        calendar.entries[dayKey] = updatedEntry
      } else {
        calendar.entries.removeValue(forKey: dayKey)
      }
    }
  }

  private func finishCalendarMutationPublishingSnapshot(
    calendarId: UUID,
    in context: ModelContext,
    mutate: (inout CustomCalendar) -> Void
  ) throws {
    try persistChanges(in: context)
    invalidatePendingReloads()

    var calendars = snapshot.calendars
  ...
  private func finishHabitMutationPublishingSnapshot(
    ...
    invalidatePendingReloads()
    let nextVersion = reserveNextDataVersion()
    publishSnapshot(calendars: calendars, dataVersion: nextVersion)
    WidgetReload.scheduleHabitWidgetsReload()
  }
  ...
  private func publishSnapshot(
    ...
    if let dataVersion, dataVersion < snapshot.dataVersion {
      return
    }
