...
- Tests sit alongside targets: unit-style coverage in `My YearTests/`, UI flows in `My YearUITests/`, and widget previews under each widget folder’s `Assets.xcassets`.
...
- Prefer `UpperCamelCase` for types, `lowerCamelCase` for properties/functions, and suffix view structs with `View` (`ProgressRingView`).
