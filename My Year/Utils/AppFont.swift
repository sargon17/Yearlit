...
enum AppFont {
  ...
  static func pixelCircle(_ size: CGFloat) -> Font {
    custom(.pixelCircle, size: size, weight: .regular)
  }

  static func custom(_ family: Family, size: CGFloat, weight: Font.Weight = .regular) -> Font {
    Font.custom(postScriptName(for: family, weight: weight), size: size)
  }

  #if canImport(UIKit)
    static func uiFont(_ family: Family, size: CGFloat, weight: Font.Weight = .regular) -> UIFont {
